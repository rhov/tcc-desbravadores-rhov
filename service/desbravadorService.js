...
function criarDesbravador({ nome, idade, documento, sexo, clubeId, unidadeId }) {
  if (!nome || !idade || !documento || !sexo || !clubeId || !unidadeId) throw new Error('Todos os campos são obrigatórios');
  ...
  const unidade = clube.unidades.find(u => u.id === unidadeId);
  if (!unidade) throw new Error('Unidade não encontrada neste clube');
  ...
  const count = desbravadores.filter(d => d.clubeId === clubeId && d.unidadeId === unidadeId).length;
  if (count >= 10) throw new Error('Unidade já possui 10 desbravadores');
  const desbravador = { id: desbravadores.length + 1, nome, idade, documento, sexo, clubeId, unidadeId };
  desbravadores.push(desbravador);
  return desbravador;
}

function listarDesbravadoresPorUnidade(clubeId, unidadeId) {
  return desbravadores.filter(d => d.clubeId === clubeId && d.unidadeId === unidadeId);
}
